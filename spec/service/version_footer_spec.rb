...
RSpec.describe OrcidPrinceton::Service::VersionFooter, type: :service do
  ...
  describe 'info' do
    context 'with stale information' do
      before do
        described_class.revision_file = fixture_path.join('REVISION').to_s
        described_class.revisions_logfile = fixture_path.join('revisions_stale.log').to_s
    ...
    context 'with current information' do
      before do
        described_class.revision_file = fixture_path.join('REVISION').to_s
        described_class.revisions_logfile = fixture_path.join('revisions_current.log').to_s
        described_class.reset!
      end
  ...
  context 'with rollback information' do
    before do
      described_class.revision_file = fixture_path.join('REVISION').to_s
      described_class.revisions_logfile = fixture_path.join('revisions_rollback.log').to_s
      described_class.reset!
    end
    it 'detects current information' do
      info = described_class.info
